...
export function CommunityFeed({ isAuthenticated }: { isAuthenticated: boolean }) {
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {filtered.map((p) => (
              <PostCard key={p.id} post={p} isAuthenticated={isAuthenticated} />
            ))}
          </div>
        </div>
        ...
        <aside className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <AlertTriangle className="h-4 w-4 text-amber-500" /> Đang chờ kiểm duyệt
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 text-xs text-muted-foreground">
              {reportFlags.slice(0, 3).map((r) => (
                <div key={r.id} className="rounded-lg border border-border/30 p-2">
